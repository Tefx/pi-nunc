...
// usage: node scripts/observe-runner.mjs
...
const stateRoot = join(f.dir, 'nunc-live-runner');
const settings = { ...f.settings, defaultProvider: 'openai-codex', defaultModel: 'gpt-6-astra', defaultThinkingLevel: 'high' };
...
try {
  ...
  const defaults = JSON.parse(preflight.stdout); assert.equal(defaults.models[0].baseUrl, f.endpoint); assert.equal(defaults.effective.thinking, 'high');
  const execution = await run(); assert.equal(execution.code, 0, execution.stdout.slice(0, 4000) + execution.stderr);
  ...
  assert(f.requests.some(r => r.payload.reasoning?.effort === 'high'));
  ...
  result = { status: 'PROVEN_CONTROLLED', calls: report.usage.calls, nativeWorkers: report.children.length, memoryQuality: 'UNPROVEN: scripted service', evidence: f.dir };
} catch (error) { result.error = { message: error.message, stack: error.stack }; process.exitCode = 1; }
finally { await rm(stateRoot, { recursive: true, force: true }); await f.close(result); console.log(JSON.stringify(result)); }

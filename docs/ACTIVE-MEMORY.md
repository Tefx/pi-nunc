...
**2026-09-12 已接受的后续变更，待实施：** [STABLE-MEMORY.md](STABLE-MEMORY.md)将本文件的每轮尾置要求替换为内容不变时保持合法锚点，并同步修改 admission、receipt 归属与 UI。下文的“唯一尾部”“只维护 F→R→M”描述已交付且当前实现采用的合同，不再限制该新增量；CRUD、CAS、持久化、保守 receipt 公式及其他未修改边界继续保留。当前代码尚未实现固定锚点，不能将本文件的历史验收当作新布局已通过。

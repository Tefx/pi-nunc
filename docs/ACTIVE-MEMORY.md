...
**2026-09-13 稳定位置已实施：** [STABLE-MEMORY.md](STABLE-MEMORY.md)将本文件的每轮尾置要求替换为内容不变时保持合法锚点。下文的“唯一尾部”“F→R→M”描述 CRUD/CAS 历史合同；主请求发送顺序为 `F | R前 | M | R后`。CRUD、CAS、持久化、保守 receipt 公式及其他未修改边界继续保留。不能将本文件的历史尾置验收当作新布局的缓存收益证明。

...
状态：**已汇总设计，等待用户调度实施；当前代码尚未提供此能力**。

本轮授权是更新文档并新增实施计划，不是实施、产品测试或安装。既有 [Larva 指令桥接](LARVA-PROMPT.md) 先独立交付，本功能消费该结果；Larva 的 v1 接口和调度条件保持不变。所有修改限于 Nunc，不改 Pi core、已安装 Pi 包、私有宿主状态或外仓。
...
笔记是 session 工作数据。工具说明提示记录已确认决策、必要理由、当前阻塞/恢复入口，并标明推测与未完成状态；编辑笔记不改变原用户指令或历史事实。不以特定措辞、笔记频率或模型复述作为行为覆盖。

...
- `active: ActiveEntry[]`: the complete, current effective main-model projection, grouped by original visible host entry ID, in order. Each entry carries its original `sourceRole` and one or more public pi-ai `Message`s. Include waiting new user messages, complete tool bodies, call arguments, result association/error status, visible bash/custom/branch-summary projections, and thinking visible in that projection. Exclude old compaction projections and non-visible metadata. The engine never reads a log, file attachment, retired history or task artifact to supplement this input.
- Optional `eligibleKeptEntryIds`: restrict host-valid start IDs before choosing K. Intersected with engine tool-pair legality; it cannot legalize an orphaned result. This supports Pi paths where a retained range must start after an older compaction entry to avoid replaying that old summary. Empty eligibility produces explicit capacity failure. Unknown/duplicate IDs fail input validation.
...
There is no second scheduler. The adapter supplies `triggerTokens = effective contextWindow - Pi compaction.reserveTokens`; it must also check Pi's `keepRecentTokens` preparation can enter the hook. An illustrative engine configuration is:

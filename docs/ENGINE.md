...
The optional main-only `nativeOutputReserve` selects adapter-owned input headroom for a verified context-clamped native route. It must be a positive integer within `outputTokens` and cover the serializer floor. The stock adapter derives it from Pi's compaction reserve (see [PI.md](PI.md)); `outputTokens` then reports the native upper bound, not a simultaneous output reservation. Different input estimators prevent a joint input-plus-native-output guarantee. Absent this field, the engine retains fixed-output accounting. Raw extraction and uncapped APIs reject it. Admission, maintenance planning and calibration share `inputLimit()` so they use the same headroom. Measured normal-at-H and actual extraction checks remain binding.

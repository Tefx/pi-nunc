...
The pi-heuristic estimator uses Pi estimateTextTokens (UTF-16 characters/4) plus explicit request/message/block framing and per-image planning estimates. Images use Pi's `estimateTextAndImageContentTokens` heuristic unless `imageTokens` overrides it; neither value is a tokenizer bound. Opaque signatures are not text tokens. Maintenance and candidate M/K always receive a fresh estimate; historical assistant usage is never reused for a newly shaped prefix. Main admission can use Pi usage-backed accounting only after a prior delegated request with the same model, system/tools and unchanged message prefix. Native JSON-enumerable snapshots exclude tool executors. Compaction, session/path/model changes and payload transformations invalidate usage attribution. Missing/malformed usage falls back to a fresh estimate; cached tokens count. Estimates, output reserves and safety margins are planning values, not hard tokenizer bounds. Provider overflow and length recovery remain native.
...
Images are supported as real native blocks when the current model advertises image input. Missing `imageTokens` uses Pi's per-image heuristic (currently 1200 tokens in Pi 0.86.1); an explicit positive override remains authoritative for planning. The same estimate applies to user/tool-result images, main admission, extraction, retained-history planning and the Context panel, in addition to Nunc's block framing. Compatible main-request usage receipts cover the historical prefix; only new trailing images need estimates. Model/prefix changes and compaction retain their existing receipt invalidation rules; extraction always estimates its newly shaped request.

This replaces the requirement for a provider-specific image upper bound. Estimates may overcount or undercount; native provider overflow recovery remains Pi-owned, and Nunc adds no automatic resend or recursive maintenance. Failed extraction preserves saved memory/history. Unsupported image models, PDF/audio/unknown blocks or unsettled source messages still produce explicit input limitations. No image is dropped, truncated or replaced with a text placeholder. Provider validation of actual image bytes remains provider-owned; fixtures establish transport preservation, not vision understanding.

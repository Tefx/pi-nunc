...
- c1/late-d: After the frozen maintenance extraction is observed, one public RPC `steer` is accepted while compact is still outstanding. D is absent from that frozen source. The runner records the accepted steer and never reissues it as `prompt` or `clear_queue`. Native continuation or a later ordinary turn drains the queued D so the first post-freeze main request includes D once. Missing overlap, including a race where maintenance ends before the steer ACK, stays `UNPROVEN`. Cancelled maintenance cannot continue later effects.

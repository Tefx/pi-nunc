...
The orchestration worker retains the supervisor's `TMPDIR` (including its absence), so both validate targets against the same allowed temporary root. The native task host alone redirects temporary files into `<stateRoot>/tmp`; no additional filesystem roots are allowed.

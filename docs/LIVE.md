...
- c4/capacity: Keep the original giant source. Observe bounded extraction omission/failure and unchanged state after cancellation; failed continuation remains unproven. A 1M native window may fit that source; overflow observation needs a strictly smaller native model such as `openrouter/mistralai/mistral-nemo`, bound by the downstream owner.

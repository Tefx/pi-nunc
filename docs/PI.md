...
Nunc captures the current public native `Provider` through `ModelRegistry.getProvider`, wraps its `stream`/`streamSimple`, and delegates to the captured original transport. For the inspected Pi 0.85.1 Completions, Responses, Azure Responses and Anthropic adapters, stock `streamSimple` sizes output from remaining context. Nunc preserves those native defaults and uses `min(model.maxTokens, compaction.reserveTokens)` as **main input-admission headroom**, floored at the serializer's minimum (16 for Responses, 1 otherwise). It does not subtract the entire catalog maximum from the window: Grok's 500,000 context / 500,000 output capability remains usable. With reserve 16,384 and Nunc safety 1,024, main safe input is 482,592; a narrower configured input limit still applies. When output capability exceeds the reserve, admission can trigger native recovery at H minus Nunc safety. Custom providers sharing these API identities must preserve the native context-clamping contract. Other adapters still delegate with fixed-output accounting until their context sizing is verified. Extraction has its separate raw-call ceiling. Native `openai-codex-responses` omits a serialized output cap; some `openai-responses` models opt out through `compat.supportsMaxOutputTokens === false`. Those requests reserve the complete `model.maxTokens` allowance. A smaller extraction allowance is invalid rather than an enforceable cap. Main admission and Pi output sizing use different estimators; the reported main `outputTokens` is an upper bound, not a simultaneous reservation. Nunc does not guarantee that its conservative input estimate plus Pi's serialized output cap fits the window. Pi/provider retain output sizing and overflow handling. Extraction still checks combined input, fixed output and safety; normal-at-H and actual full-extraction checks remain necessary.
...
Observe-only payload callbacks and public header hooks remain composed. On main-session and Nunc maintenance requests, payload mutation is rejected by a wrapper around the final native `onPayload` callback before HTTP. Independent calls keep the original `onPayload`. A later public provider registration is captured on the next context; teardown restores a predecessor only while Nunc still owns that registration. The extension does not replace host authentication. Codex OAuth, refresh, request compression and transport remain native. Its subscription cost placeholders do not establish USD billing; Nunc maintenance observations report billing as unknown. The bounded runner uses the native SSE path and full output reservations, without adding `max_output_tokens`.
...
- Admission captures the registered native `Provider` and delegates `stream`/`streamSimple`. It does not maintain an API-name allowlist. Live observer transport binds HTTP to the authorized `model.baseUrl` origin and path prefix, requires POST/SSE/single-send/no-retry/no-redirect, and checks the actual serialized output cap when present (otherwise the full `model.maxTokens` reservation). A missing `baseUrl` fails as a missing destination field. Google Generative AI's public adapter rejects custom `fetch`, so the observer cannot intercept that adapter's HTTP; admission still delegates it. Codex OAuth remains a native route when that model is selected.
- One custom compaction owner. Legacy stream overrides, raw sampling payload overrides, constrained tool sampling (absent from public maintenance `ToolInfo`), per-main-call `maxTokens` overrides on the main-session path, payload rewriting of main/maintenance requests, and provider changes during a prepared main or maintenance request are unsupported. Other extensions may add or drop their own context messages; load order is not a Nunc requirement. Arbitrary deletion or rewrite of Nunc memory carriers is unsupported and is not fully detected. Registering a replacement between settled requests is tested; arbitrary concurrent registration cannot establish compatibility.

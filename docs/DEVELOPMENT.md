...
| `tests/pi` component fixtures | Public package/extension loading, hooks, native JSONL/path rebuilding, source/media/config changes, cancellation, real isolated append failure, manual M save/restore through public `appendEntry` (unique effective M, freeze-to-native-terminal exclusion, revision conflicts, over-budget reduction, unconfirmed writes), read-only Context layout/observation (`tests/pi/context-view.test.ts`: F/M/R counts, last-main vs maintenance, planned vs enforced admission, payload appends, path/reload cleanup), compact footer formatting, `/nunc` `details`-only completions and complete shared text report (`tests/pi/reporting.test.ts`: removed status, identical non-TUI bare/details, ContextSurface budgets/maintenance accounting, diagnostic bound/self-echo, engine vs native, no-model/uncapped/zero labels), overlay list/search/edit/delete/context-jump input (`tests/pi/overlay.test.ts`), and request-bound usage receipts (`tests/pi/request-receipts.test.ts`: public `pi.sendMessage` idle custom → real tool → second callback, earlier compatible anchor vs newer incompatible low usage, public context rewrite of returned usage/content, trailing/payload growth, fresh fallback, real over-limit zero HTTP, failed/unissued responses, bounded eviction, compaction/manual-M/model/session/cancel invalidation). Their SDK fixtures supplement stock-host evidence. |
...
| `scripts/observe-ui.mjs` via `tests/pi/ui-panel.test.ts` | Stock PTY/TUI: footer, overlay search/edit/native save, slot-editor CJK paste, delete confirm, `/nunc details` complete report, fresh `/nunc status`/unknown diagnostic events, Context Diagnostics enter/return/close reading the status warning on the current overlay frame (normal and short 12×40), inflight HTTP preserved, main editor not overwritten by overlay, short browse+edit controls. PTY CJK bytes do not prove IME candidate-window geometry; human host confirmation of IME/resize/theme was recorded for the delivered UI. |
| `tests/pi/overlay-contract.test.ts` | Rejected/occupied submit keeps draft; edit revision stays frozen across live refresh; Context arrows vs preview paging; last-main/maintenance drilldown; Budget maintenance input/zero/none; Diagnostics navigation and warning recovery; short/multiline cursor+hints; unconfirmed footer; injected cancel binding; native Markdown slot preview; Context abbreviations legend (F/M/R/B/K/D). |
...
`print` emits a task-owned `HOME` / `PI_CODING_AGENT_DIR`, dummy loopback key, and a session that already contains slot `s12`. Run the printed command in a real terminal. F7 opens the overlay without consuming the main editor; Enter `s12` and type with a host IME. Confirm the candidate window tracks the overlay cursor. Esc discards. Do not send a model request. Teardown is the `rm -rf` line in `LAUNCH.txt`. Human host confirmation of IME candidate-window tracking, real-window resize and theme was recorded for the delivered UI; PTY CJK injection still does not prove those observations.

...
| `tests/pi` component fixtures | Public package/extension loading, hooks, native JSONL/path rebuilding, source/media/config changes, cancellation, real isolated append failure, and manual M save/restore through public `appendEntry` (unique effective M, freeze-to-native-terminal exclusion, revision conflicts, over-budget reduction, unconfirmed writes). Their SDK fixtures supplement stock-host evidence. |

...
| `scripts/observe-payload.mjs` via `tests/pi/payload.test.ts` | Stock loader `before_provider_request` order: observe/identity/in-place/replacement metadata reach controlled HTTP; overcap, non-stream, input growth and model rewrite send nothing. JSON-byte equality ignores prototypes. |

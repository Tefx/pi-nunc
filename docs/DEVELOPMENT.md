...
| `scripts/observe-ui.mjs` via `tests/pi/ui-panel.test.ts` | Stock PTY/TUI: footer, overlay search/edit/native save, slot-editor CJK paste, delete confirm, `/nunc details` complete report, `/nunc status`/unknown usage, Context Diagnostics navigation, inflight HTTP preserved, main editor not overwritten by overlay, short 12×40 browse+Diagnostics+edit controls. PTY CJK bytes do not prove IME candidate-window geometry; human host confirmation of IME/resize/theme was recorded for the delivered UI. |

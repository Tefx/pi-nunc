...
| `tests/live` | Native default-resolution and saved vs invoking-runtime switch, no-call preflight, target/build binding, quotas/failure/unknown usage, Completions/Responses/Anthropic serializer seams, actual stock RPC/tools/calibrated rollovers/restart, c4/full vs original capacity isolation, complete giant exposure/extraction/retirement, late-D cache-refusal scoring without a hidden direct route, cancelled-maintenance non-continuation, child termination, cleanup and prerequisite-gated artifact checks. |

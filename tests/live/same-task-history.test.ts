...
import { sameTaskHistoryChecks, sameTaskHistoryEffects, taskRead } from "../../src/live/task-retention.js";
...
const VALID_RECORDS_JSON = JSON.stringify({
  north: [{ gross: 10, refunds: 3, cost: 9 }, { gross: 0, refunds: 0, cost: 0 }],
  south: [{ gross: null, refunds: 2, cost: 1 }],
  west: [{ gross: -2, refunds: 1, cost: 4 }, { gross: 8, refunds: null, cost: 0 }],
}, null, 2);
...
function makeValidActions(scenario: ScenarioInput) {
  ...
  effect("setup", "c5", "write", { path: "records.json", content: VALID_RECORDS_JSON }, false, "Wrote records.json");
  actions.push({ turn: "setup", event: { type: "turn_complete", stopReason: "stop" } });

  // Turn preflight: validate records.json
  effect("preflight", "c6", "bash", { command: "python3 scripts/validate_records.py" }, false, "Validation succeeded: 5 total records across all regions conform to schema.");
  ...
  effect("task", "c12", "write", { path: "test_solution.py", content: metricsTests }, false, "Wrote test_solution.py");
...
test("sameTaskHistoryEffects verifies complete preliminary ingestion and rejects negative deviations", async () => {
  ...
  // 2. Negative: missing read of inputs/west.json
  const missingWest = validActions.filter(a => a.event?.toolCallId !== "c4");
  const failMissing = sameTaskHistoryEffects(scenario, missingWest, repository);
  assert.equal(failMissing.status, "UNPROVEN");

  // 3. Negative: incomplete read (content differs from fixture)
  ...
  // 4. Negative: invalid records.json data (wrong gross/refunds/cost or missing region)
  const badRecords = structuredClone(validActions);
  const c5Call = badRecords.find(a => a.event?.type === "tool_call" && a.event.toolCallId === "c5");
  c5Call.event.input.content = JSON.stringify({ north: [{ gross: 999, refunds: 0, cost: 0 }] });
  assert.equal(sameTaskHistoryEffects(scenario, badRecords, repository).status, "UNPROVEN");

  // 5. Negative: records.json written before reads completed
  const earlyWrite = structuredClone(validActions);
  const writeIdx = earlyWrite.findIndex(a => a.event?.toolCallId === "c5");
  const writeItem = earlyWrite.splice(writeIdx, 2);
  earlyWrite.unshift(...writeItem);
  assert.equal(sameTaskHistoryEffects(scenario, earlyWrite, repository).status, "UNPROVEN");

  // 6. Negative: premature read of TASK.md during setup
  ...
  // 7. Negative: padding / fake logs cannot substitute for records.json
  const fakePadding = structuredClone(validActions);
  const padCall = fakePadding.find(a => a.event?.toolCallId === "c5");
  padCall.event.input.path = "archive.json";
  assert.equal(sameTaskHistoryEffects(scenario, fakePadding, repository).status, "UNPROVEN");
});
...
test("sameTaskHistoryChecks verifies 3-compaction series, first read in K, and rejects corruptions", async () => {
  ...
  const branch: any[] = [
    ...
    { id: "a-setup", parentId: "u-setup", type: "message", message: { role: "assistant", content: [{ type: "toolCall", id: "c5", name: "write", arguments: { path: "records.json", content: VALID_RECORDS_JSON } }], timestamp: 2, api: "openai-completions", provider: "openrouter", model: "openai/gpt-5.6-luna", usage: { input: 1, output: 1, totalTokens: 2 }, stopReason: "toolUse" }, timestamp: "2" },
    ...
    { id: "a-test", parentId: "r-sol", type: "message", message: { role: "assistant", content: [{ type: "toolCall", id: "c12", name: "write", arguments: { path: "test_solution.py", content: metricsTests } }], timestamp: 12, api: "openai-completions", provider: "openrouter", model: "openai/gpt-5.6-luna", usage: { input: 1, output: 1, totalTokens: 2 }, stopReason: "toolUse" }, timestamp: "12" },
  ...
  // Set memory slot projection on rebuilt
  (rollovers[1].rebuilt[0] as any).slot = mem1;
  const requests: any[] = [
  ...
  // Mock project to return mem1 for rollovers[1]
  const mockRebuiltSlots = (r: any) => {
    return r === rollovers[1].rebuilt ? { memory: { slots: mem1 } } : { memory: { slots: [] } };
  };
  ...
  // 1. Requirements entered through designated request & read following setup
  assert.equal(checks[0]!.status, "PROVEN");
  // 2. Preliminary effects verified
  assert.equal(checks[1]!.status, "PROVEN");
  // 3. Observed build failure and repair
  assert.equal(checks[3]!.status, "PROVEN");
  // 4. Input/oracle separation is semantic
  assert.equal(checks[4]!.status, "UNPROVEN");
...
test("diagnosePreparationFeasibility correctly assesses real deficit on short history and proves feasibility on same-task-history", async () => {
  const { input: scenario } = await loadScenario(repository, { id: "g4", variant: "same-task-history", config });
  ...
  // 1. Negative: Sole-user-request short history (B = 12 tokens, H = 5220, F = 1855, M = 336, G = 1024, K = 4645 -> deficit = 2640 tokens)
  ...
  assert(shortDiag.deficitTokens > 0, "Deficit must be positive for short sole-request history");
  assert.match(shortDiag.explanation, /cannot pay fixed\/memory\/growth costs/);

  // 2. Positive: Multi-turn same-task-history with genuine prefix (B >= 2500 tokens, LHS <= H)
  const prefixText = "x".repeat(10000); // 10000 chars ~ 2500 tokens of preliminary ingest reads
  ...
  const longDiag = await diagnosePreparationFeasibility({
    ...
    fixed,
    repository,
    trigger: { callId: "call_task3", requestText: scenario.turns[2]!.text, path: "TASK.md", cwd: "/task", fixtureContent: scenario.files["TASK.md"]!, contextTokens: 5500 }
  });
  assert.equal(longDiag.feasible, true);
  assert.equal(longDiag.status, "FEASIBLE");
  assert.equal(longDiag.deficitTokens, 0);
...
async function sameTaskHistoryHost(group: "native" | "candidate", tools = true, lowUsage = false) {
  ...
  const turnSetup = [
    ...
    tool("write", { path: "records.json", content: VALID_RECORDS_JSON }),

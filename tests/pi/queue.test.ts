...
for (const mode of ["rpc", "tui"] as const) test(`bounded downstream stdin -> actual stock ${mode}: freeze, overflow, queues/editor and cancellation`, { timeout: 115000 }, async () => {
  ...
  try {
    ...
    console.log(JSON.stringify({ mode, observed: report.stock[0].status, requests: report.stock[0].requests, cleanup: report.cleanup, memoryQuality: "UNPROVEN (controlled service)" }));

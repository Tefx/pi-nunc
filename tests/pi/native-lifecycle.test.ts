...
for (const api of ["openai-responses", "anthropic-messages", "openai-codex-responses"]) test(`stock ${api} adapter: native threshold maintenance and saved-context continuation`, { timeout: 100000 }, () => {
...
test("stock Codex planning overruns delegate; explicit and model limits still reject before HTTP", { timeout: 110000 }, () => {
  const root = fileURLToPath(new URL("../../../", import.meta.url));
  const result = spawnSync(process.execPath, ["scripts/observe-capacity-recurrence.mjs"], { cwd: root, encoding: "utf8", timeout: 100000, maxBuffer: 2_000_000, env: { PATH: "/opt/homebrew/bin:/usr/bin:/bin", PI_OFFLINE: "1", PI_SKIP_VERSION_CHECK: "1", PI_TELEMETRY: "0" } });
  assert.equal(result.status, 0, result.stdout + result.stderr + String(result.error ?? "")); console.log(result.stdout.trim());
});
test("stock Codex defaults admit real input and persist uncapped output above planning reserve", { timeout: 100000 }, () => {
  const root = fileURLToPath(new URL("../../../", import.meta.url));
  const result = spawnSync(process.execPath, ["scripts/observe-capacity-defaults.mjs"], { cwd: root, encoding: "utf8", timeout: 90000, maxBuffer: 2_000_000, env: { PATH: "/opt/homebrew/bin:/usr/bin:/bin", PI_OFFLINE: "1", PI_SKIP_VERSION_CHECK: "1", PI_TELEMETRY: "0" } });
  assert.equal(result.status, 0, result.stdout + result.stderr + String(result.error ?? "")); console.log(result.stdout.trim());
});
